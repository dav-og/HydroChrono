...
int main(int argc, char* argv[]) {
    std::vector<double> stiffness_values = {1.00E+06};
    std::vector<double> damping_values   = {1.25E+06};
    //{1.75E+06, 2.0E+06}; // 0.25E+06, 0.50E+06, 0.75E+06, 1.0E+06, 
    std::vector<double> wave_periods     = {5.0, 6.0, 7.0, 7.25, 7.5, 7.75, 8.0, 9.0, 10.0};  // wave periods in seconds
    for (double current_stiffness : stiffness_values) {
        for (double current_damping : damping_values)
            for (double current_period : wave_periods) {
            ...
            auto hull_mesh_fname =
                (DATADIR / "talos" / "geometry" / "TALOS_G1_half.obj").lexically_normal().generic_string();
            ...
            double simulation_duration = 320.0;
            ...
            double irm_mass = total_mass * 0.5;
            irm->SetMass(irm_mass);
            double radius      = 2.5;
            double irm_inertia = (2.0 / 5.0) * irm_mass * pow(radius, 2);
            irm->SetInertiaXX(ChVector<>(irm_inertia, irm_inertia, irm_inertia));
            irm->SetPos(ChVector<>(0.0, 0.0, -5.7));

            // PTO parameters
            double pto_length_bottom = 5.0;
            double pto_length_top    = 2.0;
            double pto_stiffness     = current_stiffness;
            double pto_damping       = current_damping;
            ...
            spring_1->Initialize(hull, irm, true, ChVector<>(5.000, 0.000, -8.660), ChVector<>(2.500, 0.000, -4.330));
            spring_1->SetRestLength(pto_length_bottom);
            ...
            spring_2->Initialize(hull, irm, true, ChVector<>(-2.500, 4.330, -8.660), ChVector<>(-1.250, 2.165, -4.330));
            spring_2->SetRestLength(pto_length_bottom);
            ...
            spring_3->Initialize(hull, irm, true, ChVector<>(-2.500, -4.330, -8.660),
                                 ChVector<>(-1.250, -2.165, -4.330));
            spring_3->SetRestLength(pto_length_bottom);
            ...
            spring_4->Initialize(hull, irm, true, ChVector<>(5.000, 0.000, 8.660), ChVector<>(2.500, 0.000, 4.330));
            spring_4->SetRestLength(pto_length_top);
            ...
            spring_5->Initialize(hull, irm, true, ChVector<>(-2.500, 4.330, 8.660), ChVector<>(-1.250, 2.165, 4.330));
            spring_5->SetRestLength(pto_length_top);
            ...
            spring_6->Initialize(hull, irm, true, ChVector<>(-2.500, -4.330, 8.660), ChVector<>(-1.250, -2.165, 4.330));
            spring_6->SetRestLength(pto_length_top);
            ...
            // my_hydro_inputs->ramp_duration       = simulation_duration*0.1;
             ...
             double wave_omega                       = 2.0 * M_PI / current_period;
             my_hydro_inputs->regular_wave_omega     = wave_omega;  // 1.046778671;  // 1.427996661;
            ...
            if (visualization_on) {
                ...
                while (irrlicht_vis->Run() && system.GetChTime() <= simulation_duration) {
                    ...
                    if (buttonPressed) {
                        ...
                        irm_heave.push_back(irm->GetPos().z());
...
#endif
            if (save_data_on) {
                // Modify the directory where results are saved to include the stiffness and damping value
                std::ostringstream oss_stiffness, oss_damping, oss_period;
                oss_stiffness << std::setprecision(1) << std::scientific << current_stiffness;
                oss_damping << std::setprecision(1) << std::scientific << current_damping;
                oss_period << std::fixed << std::setprecision(2) << current_period;  // using fixed notation here

                std::string stiffness_str = oss_stiffness.str();
                std::string damping_str   = oss_damping.str();
                std::string period_str    = oss_period.str();

                // Replace 'e+' with 'E' for stiffness and damping
                std::size_t found_stiffness = stiffness_str.find("e+");
                std::size_t found_damping   = damping_str.find("e+");

                if (found_stiffness != std::string::npos) {
                    stiffness_str.replace(found_stiffness, 2, "E");
                }

                if (found_damping != std::string::npos) {
                    damping_str.replace(found_damping, 2, "E");
                }

                // Replace '.' with '_'
                std::replace(stiffness_str.begin(), stiffness_str.end(), '.', '_');
                std::replace(damping_str.begin(), damping_str.end(), '.', '_');
                std::replace(period_str.begin(), period_str.end(), '.', '_');  // this will replace '.' in '5.00'
                // with '_'

                std::string base_path = "./results/talos/mass_50_50/pto_k/" + stiffness_str + "/pto_c/" + damping_str + "/" + period_str + "/";
                ...
                for (int i = 0; i < time_vector.size(); ++i) {
                    ...
                    output_file << std::fixed << std::setprecision(2) << time_vector[i] << "," << std::fixed
                                ...
                                << hull_heave[i] << "," << std::fixed << std::setprecision(8) << hull_pitch[i] << ","
                                << std::fixed << std::setprecision(8) << irm_heave[i] << std::endl;
                    ...
                    for (int j = 0; j < 6; ++j) {
                        power_output_file << "," << std::fixed << std::setprecision(8)
                                          << power_absorbed_by_spring[j][i];
                    }
                    power_output_file << "," << std::fixed << std::setprecision(8) << total_power_absorbed[i]
                                      << std::endl;
                }
                output_file.close();
